<script lang="ts">
  ...
  export let forkConversation: ((messageId: number, branchName?: string) => Promise<void>) | null = null;
      ...
      <div class="space-y-3">
        {#each messages as message, index (message.content + index)}
          <ChatMessage 
            ...
            canFork={isAuthenticated && currentConversationId && message.id && index < messages.length - 1}
