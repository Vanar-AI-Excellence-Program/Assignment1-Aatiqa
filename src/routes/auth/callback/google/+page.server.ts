...
export const load: PageServerLoad = async ({ url, cookies }) => {
  try {
    ...
    if (error) {
      ...
      const redirectUrl = context === 'login' ? '/signup/login?error=oauth_cancelled' : '/signup?error=oauth_cancelled';
      throw redirect(303, redirectUrl);
    }

    if (!code) {
      const redirectUrl = context === 'login' ? '/signup/login?error=oauth_failed' : '/signup?error=oauth_failed';
      throw redirect(303, redirectUrl);
    }

    // Exchange code for user info
    const googleUser = await exchangeGoogleCode(code, context as 'signup' | 'login');

    if (!googleUser.email || !googleUser.name) {
      const redirectUrl = context === 'login' ? '/signup/login?error=oauth_incomplete' : '/signup?error=oauth_incomplete';
      throw redirect(303, redirectUrl);
    }
      ...
      if (existingUser.length === 0) {
        ...
        const newUser = await db
          ...
          .values({
            ...
            password: 'oauth_google', // Special marker for OAuth users
            status: 'active',
        ...
        try {
          await sendVerificationEmail(googleUser.email, googleUser.name, verificationToken, 'google');
        } catch (emailError) {
        ...
        throw redirect(303, `/auth/verification-pending?email=${encodeURIComponent(googleUser.email)}&provider=google`);
      } else {
    ...
    // Redirect to dashboard (let the smart router handle user vs admin routing)
